...
void MainForm::getUserID() {
    ...
    if (geteuid() == 0) {
        ...
        for (int pwTry = 0; pwTry < 3; pwTry++) {
            if (pwTry == 0) pwDialog1.setPassPrompt("第一次尝试：");
            else if (pwTry == 1) pwDialog1.setPassPrompt("第二次尝试：");
            else pwDialog1.setPassPrompt("最后机会！");
            // pwDialog1.show();
            pwDialog1.exec();
        ...
        if (pwStatus == BAD || pwStatus == CANCELLED) {
            MainForm::mainFormWidget.runBtn->setChecked(false);
            QMessageBox::warning(0, "密码失败", "无法验证密码");
            exit(1);
        }
    }
}
...
void MainForm::addInterfaces() {
    ...
    if (system(commandLine.c_str()) == 0) {
        ...
        QMessageBox::critical(0, "糟糕", QString("无法继续。\n无法找到接口管道"));
        ifs.close();
...
void MainForm::doRun() {
    if (MainForm::mainFormWidget.runBtn->isChecked()) {
        ...
        MainForm::mainFormWidget.statusTxt->setText("正在扫描 ...");
        ...
        MainForm::mainFormWidget.statusTxt->setText("等待wifi终止扫描 ...");    
    }
}
...
//    cout << "closeEvent intercepted..." << endl;
    ...
    MainForm::mainFormWidget.statusTxt->setText("正在退出，等待wifi驱动 ...");
    MainForm::mainFormWidget.statusTxt->repaint();
    pGetterThread->QThread::quit();
    writePrefsFile();
    pGetterThread->QThread::wait();
    MainForm::mainFormWidget.statusTxt->setText("正在关闭 ...");
    MainForm::mainFormWidget.statusTxt->repaint();
...
void MainForm::drawTable() {
    ...
    MainForm::mainFormWidget.mainTableWidget->setHorizontalHeaderLabels(
            QString("绘图|SSID|MAC|频道|模式|安全|隐私|加密算法|频率\
|质量|信号|频宽 MHz|最小信号|最大信号|中心频道|首次发现|最后发现|厂商|协议|类型").split("|"));
    setVisibleCols();
...
void MainForm::initNewCell(string macAddress, int tbi) {
    ...
    MainForm::cellDataRay[tbi].essid = "<隐藏>"; // nl80211 iw doesn't report SSID line if hidden
    MainForm::cellDataRay[tbi].minSignal = 0;
    MainForm::cellDataRay[tbi].maxSignal = -120;
    MainForm::cellDataRay[tbi].firstSeen = now;
    MainForm::cellDataRay[tbi].protocol = "未知";
    ...
    while (!lastLine && getline(thePipe, tempLine)) {
        ...
        if (lastLine) {
            ...
            if (MainForm::mainFormWidget.runBtn->isChecked()) {
                ...
                MainForm::mainFormWidget.statusTxt->setText("暂停");
            }
            if (block >= 0) {
                MainForm::fillTable();
                MainForm::fillPlots();
